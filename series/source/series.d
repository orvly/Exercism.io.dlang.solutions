...
pure const(int[]) digits(string s)
{
    import std.algorithm.iteration, std.conv, std.array, std.uni, std.exception, std.format;

    auto digitToInt(dchar c) {
        enforce(isNumber(c), format("%s is not a number", c));
        return to!int(c - '0');
    }
    return s.map!(digitToInt).array;
}

pure const(int[][]) slice(string s, int n)
{
    import std.range, std.array, std.exception;
    enforce(n <= s.length, "n must be lower or equal than string length");
    return s.digits.slide(n).array;
}
